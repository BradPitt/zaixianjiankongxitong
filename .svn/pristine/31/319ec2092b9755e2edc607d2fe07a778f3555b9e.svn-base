...
function setquxian(stationName,updata){
	...
	// $("#curveData").height($("#curveData").height() +2);
	 $(".tegContent").height($(".tegContent").height() - 2);
    // 曲线数据
    if(!updata){
        showLoad();
        var quxian = {
        ...
        quxian.option = {
            ...
            grid: {
                ...
                right: 20,
                bottom: 30,
                height: $(curveData).height() - $(".tagTag").height()-80 ,
                width: $(".waterBox div.center .tagBox ul.tegContent").width() ,
        ...
        siteOption.panels.forEach(function(item, index) {
            $(curveTag).append('<li ' +(index == 0?"class='active'":"")+ '>' + item.cnName + '</li> ');
            ...
            $(curveData).find("div.canvas").height($(curveData).height())
